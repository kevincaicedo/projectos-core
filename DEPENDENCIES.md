...
| `tempfile` | dev-only: store/log/CLI test fixtures | Test isolation directories stop working; no production code path is affected | Replace with a small fixture helper over `std::env::temp_dir` plus cleanup | Correct cross-platform unique-dir creation with drop cleanup is subtle (races, leak-on-panic); the crate is the community answer | founders |
